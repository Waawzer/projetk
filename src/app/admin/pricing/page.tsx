...
import { FiPlus, FiEdit2, FiTrash2, FiArrowUp, FiArrowDown, FiStar } from 'react-icons/fi';
...
export default function PricingPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/pricing?id=${planToDelete._id}`, {
        method: 'DELETE',
      });
      ...
      setPricingPlans(pricingPlans.filter(plan => plan._id !== planToDelete._id));
      ...
      setError('Erreur lors de la suppression du tarif. Veuillez réessayer plus tard.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleMoveUp = async (planId: string) => {
    ...
    try {
      ...
      setError('Erreur lors de la modification de l\'ordre. Veuillez réessayer plus tard.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-2">Gérez les tarifs affichés sur votre site</p>
        </div>
        <Link
      ...
      {pricingPlans.length === 0 ? (
        <div className="bg-card border border-gray-800 rounded-xl p-12 text-center shadow-xl">
          ...
          <h3 className="text-xl font-medium text-gray-200">Aucun tarif trouvé</h3>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-800">
                {pricingPlans.map((plan) => (
                  <tr key={plan._id} className="group hover:bg-background/50 transition-colors duration-200">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-200">{plan.title}</div>
                      <div className="text-xs text-gray-400 mt-1 max-w-xs truncate">{plan.description}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-gray-300">{plan.order}</span>
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-gray-400">
                        Êtes-vous sûr de vouloir supprimer le tarif "{planToDelete.title}" ? Cette action est irréversible.
                      </p>
                    </div>
                  </div>
                </div>
